...
class ErrorHandler {
    ...
    async handleError(error, context) {
        ...
        if (this.screenshotOnError && context.page && this.resultManager) {
            try {
                const errorScreenshot = await this.resultManager.saveScreenshot(context.page, context.runDir, `error_${Date.now()}`);
    ...
    async retryOperation(operation, context) {
        ...
        for (let attempt = 1; attempt <= this.retryCount; attempt++) {
            try {
                ...
                lastError = error;
                console.log(`Retry attempt ${attempt}/${this.retryCount} failed:`, error instanceof Error ? error.message : String(error));
