...
const addToHistory = (url) => {
  ...
  if (url === navigationState.currentURL) return;
...
ipcMain.handle('results:saveScreenshot', async (event, runDir, name) => {
  try {
    ...
    if (!page) throw new Error('No current page for screenshot');
...
ipcMain.handle('performance:optimizePage', async () => {
  try {
    const page = automationService.getCurrentPage();
    if (!page) throw new Error('No active page to optimize');
