...
export class ReportGenerator {
  ...
  async generateHTMLReport(runDir: string): Promise<string> {
...
<p>Successful: ${results.filter((r: any) => r.type === 'success').length}</p>
<p>Failed: ${results.filter((r: any) => r.type === 'error').length}</p>
<h2>Actions</h2>
`;
...
`;
  ...
  async generatePDFReport(runDir: string): Promise<string> {
    const results = this.resultManager.loadResults(path.join(runDir, 'results.json'));
    if (!results) throw new Error('Results not found');
    const screenshots = fs.readdirSync(runDir)
      .filter(file => file.endsWith('.png'))
    ...
    doc.text(`Successful: ${results.filter((r: any) => r.type === 'success').length}`);
    doc.text(`Failed: ${results.filter((r: any) => r.type === 'error').length}`);
    doc.moveDown();
  ...
  async generateCSVReport(runDir: string): Promise<string> {
    const results = this.resultManager.loadResults(path.join(runDir, 'results.json'));
    if (!results) throw new Error('Results not found');
  ...
  async generateJSONReport(runDir: string): Promise<string> {
    const results = this.resultManager.loadResults(path.join(runDir, 'results.json'));
    if (!results) throw new Error('Results not found');
